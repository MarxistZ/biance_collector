...
整体目录结构使用**按市场 -> 按交易对**的方式进行分层级划分：

```text
data/
├── spot/                                          # 现货市场数据目录
│   └── {SYMBOL}/                                  # 交易对名称 (例如: BTCUSDT)
│       └── spot_{SYMBOL}_{YYYYMMDDHH}.parquet     # 现货订单簿文件
└── futures/                                       # 合约市场数据目录
    └── {SYMBOL}/                                  # 交易对名称 (例如: BTCUSDT)
        ├── future_{SYMBOL}_{YYYYMMDDHH}.parquet   # 合约订单簿文件
        └── funding_{SYMBOL}_{YYYYMMDDHH}.parquet  # 合约资金费率及统计数据文件
```
...
*   **现货订单簿文件:** `spot_{SYMBOL}_{YYYYMMDDHH}.parquet` (例如 `spot_BTCUSDT_2026030913.parquet` 表示 13:00 - 13:59 期间的数据)。如果在同一小时内重启采集器，新数据会自动追加 (concat) 到同名文件中。
*   **合约订单簿文件:** `future_{SYMBOL}_{YYYYMMDDHH}.parquet` (例如 `future_BTCUSDT_2026030913.parquet`)。
*   **资金费率文件 (仅合约专属):** `funding_{SYMBOL}_{YYYYMMDDHH}.parquet` (例如 `funding_BTCUSDT_2026030913.parquet`)。
...
### 4.1 现货订单簿表 (Spot Orderbook - 共 86 列)
...
### 4.2 合约订单簿表 (Futures Orderbook - 共 88 列)
...
### 4.3 合约资金费率综合表 (Futures Funding Rate 及市场盘口 - 共 9 列)
